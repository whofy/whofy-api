...
| `chat_service.py` | 103 | Groq LLM chatbot service (`openai/gpt-oss-20b`); system prompt defines Whofy platform knowledge; handles rate limits |
...
| File | Lines | Purpose |
|------|-------|---------|
| `settings.py` | 20 | Pydantic `BaseSettings` class; loads `.env` via `pydantic-settings`; defines `mongodb_uri`, `cors_origins`, `groq_chatbot_api_key`, `groq_resume_parser_api_key`, `adzuna_app_id`, `adzuna_app_key`, `clerk_secret_key` |
...
| `GROQ_CHATBOT_API_KEY` | ✅ `settings.groq_chatbot_api_key` | Groq Cloud (LLM inference for Chatbot: openai/gpt-oss-20b) |
| `GROQ_RESUME_PARSER_API_KEY` | ✅ `settings.groq_resume_parser_api_key` | Groq Cloud (LLM inference for Resumes: openai/gpt-oss-120b) |
...
### 10. LinkedIn Scraper Concurrency Bottleneck
While `ingest_api.py` and other sources execute much faster due to recent ThreadPoolExecutor optimizations (e.g., Adzuna and Himalayas), the `linkedin/scraper.py` module in `ingest_scrapper.py` is entirely sequential. It parses thousands of job links and iterates through them one-by-one to fetch rich HTML job descriptions with built-in `time.sleep()` delays. This causes the LinkedIn enrichment phase alone to take ~50 minutes on a single thread, heavily bottlenecking the entire ingestion pipeline.
